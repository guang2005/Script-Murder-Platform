...
*/
//拼车组队表
CREATE TABLE `script_kill_carpool` (
                                       `id` bigint NOT NULL AUTO_INCREMENT COMMENT '拼车单ID',
                                       `shop_id` bigint NOT NULL COMMENT '店铺ID（关联黑马点评店铺表）',
                                       `script_id` bigint NOT NULL COMMENT '剧本ID',
                                       `creator_user_id` bigint NOT NULL COMMENT '发起者用户ID',
                                       `target_num` int NOT NULL COMMENT '目标人数',
                                       `current_num` int NOT NULL DEFAULT 1 COMMENT '当前人数',
                                       `play_time` datetime NOT NULL COMMENT '期望游玩时间',
                                       `status` tinyint NOT NULL COMMENT '状态：1-待拼车 2-拼车中 3-拼车成功 4-拼车失败',
                                       `expire_time` datetime NOT NULL COMMENT '拼车过期时间',
                                       `user_ids` varchar(500) NOT NULL DEFAULT '' COMMENT '参与用户ID，逗号分隔（如：1001,1002,1003）',
                                       `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
                                       `update_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
                                       PRIMARY KEY (`id`),
                                       KEY `idx_shop_id_status` (`shop_id`,`status`) COMMENT '按店铺+状态查询',
                                       KEY `idx_expire_time` (`expire_time`) COMMENT '过期清理用'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='剧本杀拼车核心表（极简版）';
